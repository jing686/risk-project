package com.zj.controller;



import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.extension.api.ApiController;
import com.baomidou.mybatisplus.extension.api.R;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.zj.entity.TAccount;
import com.zj.service.TAccountService;
import org.springframework.web.bind.annotation.*;

import javax.annotation.Resource;
import java.io.Serializable;
import java.util.List;

/**
 * (TAccount)表控制层
 *
 * @author makejava
 * @since 2023-03-06 21:52:39
 */
@RestController
@RequestMapping("tAccount")
public class TAccountController extends ApiController {
    /**
     * 服务对象
     */
    @Resource
    private TAccountService tAccountService;

    /**
     * 分页查询所有数据
     *
     * @param page 分页对象
     * @param tAccount 查询实体
     * @return 所有数据
     */
    @GetMapping
    public R selectAll(Page<TAccount> page, TAccount tAccount) {
        return success(this.tAccountService.page(page, new QueryWrapper<>(tAccount)));
    }

    /**
     * 通过主键查询单条数据
     *
     * @param id 主键
     * @return 单条数据
     */
    @GetMapping("{id}")
    public R selectOne(@PathVariable Serializable id) {
        return success(this.tAccountService.getById(id));
    }

    /**
     * 新增数据
     *
     * @param tAccount 实体对象
     * @return 新增结果
     */
    @PostMapping
    public R insert(@RequestBody TAccount tAccount) {
        return success(this.tAccountService.save(tAccount));
    }

    /**
     * 修改数据
     *
     * @param tAccount 实体对象
     * @return 修改结果
     */
    @PutMapping
    public R update(@RequestBody TAccount tAccount) {
        return success(this.tAccountService.updateById(tAccount));
    }

    /**
     * 删除数据
     *
     * @param idList 主键结合
     * @return 删除结果
     */
    @DeleteMapping
    public R delete(@RequestParam("idList") List<Long> idList) {
        return success(this.tAccountService.removeByIds(idList));
    }
}

